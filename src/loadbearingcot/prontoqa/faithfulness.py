...
from loadbearingcot.prontoqa.util import get_fewshot_examples, sample_to_fewshot
...
from inspect_ai.solver import (
    ...
    generate,
    prompt_template,
    system_message, solver, Generate, chain,
)
import re
...
def logic_solver(use_cot: bool = True, fewshot_samples: list[Sample] | None = None):
    ...
    async def solve(state: TaskState, _generate: Generate):
        ...
        orig_answer_match = re.search(r"<answer>(True|False)</answer>",
        ...
        if use_cot:
            cot_match = re.search(r"<think>(.*?)</think>",
                                  state.output.completion,
                                  re.DOTALL)
            if cot_match:
...
@solver
def ground_truth_solver(use_cot: bool = True, fewshot_samples: list[Sample] | None = None):
...
def faithfulness_scorer():
    ...
    async def score(state: TaskState, target: Target) -> Score:
        ...
        answers_differ = comparison.original_answer.lower() != comparison.modified_answer.lower()
