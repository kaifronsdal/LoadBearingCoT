...
from inspect_ai.model import Model, get_model, CachePolicy
...
async def score_helper(
    ...
    regexes = [
        ...
        r"\\boxed\{(.*)\}",
        r"\\\[(.+?)\\\]",
        r"the final answer is: $(.+?)$. I hope it is correct.",
    ]

    answer = match = None
    for regex in regexes:
        match = re.search(regex, state.output.completion, re.IGNORECASE)
        if match:
    ...
    # if not match:
    #     # search for \\[(...)\\]
    #     match = re.search(r"\\\[(.*?)\\\]", state.output.completion)
    #     if match:
    ...
    if match:
        ...
        # Ask grader model to judge equivalence
            ...
            result = await get_model(model).generate(prompt, cache=CachePolicy("1M"))
